...
% x1 = log10(l/k);
% x2 = Mach Number
Vehicle.Aero.GI_Log10_CutoffReNumber_fcn_Roughness_Mach =  empModel.GI_Log10_CutoffReNumber_fcn_Roughness_Mach;

% Digitization of DATCOM figure 4.1.5.1-26 (Reader pg. 747)
% x1 = Re;
% x2 = Mach Number
Vehicle.Aero.GI_SkinFrictionCoeff_fcn_ReNumber = empModel.GI_SkinFrictionCoeff_fcn_ReNumber;
...
% Drag of cylindrical rod perpendicular to flow 
% Taken from Applied Fluid Dynamics Handbook (Pg. 334) 
% L/D or L2D is the ratio of the lenght of the cylinder to it's diameter 
% CD_fcn_L2D = [L/D CD]
CD_fcn_L2D = [...
1.0  0.64
1.98 0.68
...
Vehicle.Aero.GI_CD_fcn_L2D_FlowPerpendicularToCylinder = griddedInterpolant(CD_fcn_L2D(:,1),CD_fcn_L2D(:,2),'linear','nearest');

% Digitization of DATCOM figure 4.1.5.1-28b (Reader pg. 749)
% Lfiting-surface correlation facator for subsonic maximum drag 
% Applicable for M <= 0.25
RLS = [...
    0.50247	0.81104
    0.53517	0.83911
    0.57348	0.86534
...
Vehicle.Aero.GI_RLS_fcn_sweep_at_max_chordratio = griddedInterpolant(RLS(:,1),RLS(:,2));
end 