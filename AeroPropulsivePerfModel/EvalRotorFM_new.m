...
% FUNCTION TO CONVERGE INFLOW OVER THE ROTORS
    function [x_infl,dx_infl,iter,convflag] = ConvergeInflow(Vhub_RA,Om_RA,v_loc_BA,w_loc_BA,N,BETA,PSI_IN,PSIW,rho,SpeedOfSound,Setup)
        ...
        while(conv==0&&iter<imax)
            ...
            x_infl = x_infl_old + .4*dx_infl;

            
            %s = abs(dv_infl)<dv_tol;       % Criterion 1, use either 1 or 2
            s = abs(Xr_dot)<dv_tol;   % Criterion 2, use either 1 or 2
            conv = all(s,[1,2]);

%             if conv~=0 && Setup.nRotor ==1
%                 Xr_dot
%                 iter
%                 dv_infl
%                 iter
