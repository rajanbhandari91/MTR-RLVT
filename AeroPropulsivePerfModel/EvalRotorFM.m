function [F_RA,M_RA,Xrotor_dot,Summary,ElemWise,XInflow_out] = EvalRotorFM(Setup,FlowCon,RPM,BladePitch,XInflow,Vhub_RA,Om_RA,HealthStatus,EvalMode)
...
[~,RotorThrust,~,dT,dL,dD,Cl,Cd,ALPHADEG,PHI,RE,F_RA,M_RA,Fx_RA,Fy_RA,Fz_RA,Mx_RA,My_RA,Mz_RA,Trq,dFx_BA,dFy_BA,dFz_BA,dMx_RA,dMy_RA,dMz_RA,Loads_PPDIM,VInflow]...
    = ComputeRotorLoads(v_loc_BA,w_loc_BA,N,BETA,XInflow,PSI_IN,PSIW,rho,SpeedOfSound,Setup);
...
% PITT PETERS DYNAMIC INFLOW MODEL WITH WAKE DISTORTION MODIFICATIONS
    function [Xrot_dot,Xrot_ss] = EvalDynamicInflowModel(X_inflow,Vhub_IN,Om_IN,Loads_rotor,N,radius)
        ...
        Vm = sqrt(mu.^2 + (lam0 + Vc).^2 + 0.0000000000001);
        % mass flow parameter associated with higher harmonics of inflow
        Vbar = (mu.^2 + (lam0 + Vc).*(2*lam0 + Vc))./Vm + 0.00000000001;
              
        % Time constants for rotor dynamic wake distortion model
        % Note: Verify whether these are correct.
        TX = 32./(15*pi.*Vbar + 0.0000000000001);
        TS = 32./(15*pi.*Vm + 0.0000000000001);
        TR = 32./(15*pi.*Vbar + 0.0000000000001);
        
        %TD = diag([TX,TS,TR,TR]);
        
        % quasi-steady wake skew
        chi = atan(mu./(lam0 + Vc + 0.0000000000001));
        X_qs = tan(chi/2);
        ...
        kc_qs = qndm./(lam0 + Vc + 0.0000000000001);
        
        % quasi-steady lateral wake curvature (no flapping)
        ks_qs = pndm./(lam0 + Vc + 0.0000000000001);
...
%             V1c
%             T1=(Om.^2.*R).*CT 
%             T2=- (  Vm.*Om).*(invL_11.*Vi0   )
%             T3 = - (  Vm.*Om).*(  +  invL_12.*V1s  )
%             T4 = - (  Vm.*Om).*( invL_13.*V1c)
% 
%         end
% 
...
%         Li0_prime = (CT           - (  Vm).*(invL_11.*Li0   +  invL_12.*L1s  + invL_13.*L1c))/M(1,1);
%         L1s_prime = ((-Croll)     - (Vbar).*(invL_21.*Li0   +  invL_22.*L1s  + invL_23.*L1c))/M(2,2);
%         L1c_prime = ((-Cpitch)    - (Vbar).*(invL_31.*Li0   +  invL_32.*L1s  + invL_33.*L1c))/M(3,3);
        
%         Vi0_dot = (Li0_prime .* 1).*(Om.*R);
%         V1s_dot = (L1s_prime .* 1).*(Om.*R);
%         V1c_dot = (L1c_prime .* 1).*(Om.*R);
        ...
        Vi0_dot = (Vi0_ss - Vi0)./(tau1);
        V1s_dot = (V1s_ss - V1s)./tau2;
        V1c_dot = (V1c_ss - V1c)./tau3;
...
% FUNCTION TO CONVERGE INFLOW OVER THE ROTORS
    function [x_infl,dx_infl,iter,conv] = ConvergeInflow(Vhub_RA,Om_RA,v_loc_BA,w_loc_BA,N,BETA,PSI_IN,PSIW,rho,SpeedOfSound,Setup)
        ...
        while(conv==0&&iter<imax)
            ...
            [Xr_dot,Xr_ss] = EvalDynamicInflowModel(x_infl,Vhub_RA,Om_RA,loads_PPDIM,N,radius);
            x_infl_old = x_infl;
            x_infl_new = Xr_ss;
            relaxfactor = 0.3;

            relaxfactor(iter>60) = 0.1;

            % METHOD 1: Keep updating the inflow variables based on difference
            % between two successive iterations with a relaxation factor
            dx_infl = x_infl_new - x_infl_old;
            dv_infl = dx_infl(1:3,:);
            x_infl = x_infl_old + relaxfactor*(x_infl_new - x_infl_old);

            % % ensure first element is positive
            % x_infl(1) = max(0, x_infl(1));


            s = abs(dv_infl)<dv_tol;       % Criterion 1, use either 1 or 2
            %         s = abs(Xr_dot(1:3))<dv_tol;   % Criterion 2, use either 1 or 2
            conv = all(s,[1,2]);

            % METHOD 2: Update the inflow quantities by an amount proportional
            % to their time derivatives multiplied with a relaxation factor

            %         x_infl = x_infl_old + 0.01*Xr_dot;
            %         dx_infl = x_infl - x_infl_old;
            %         s = abs(Xr_dot(1:3))<dv_tol;
            %         conv = all(s,[1,2]);
        ...
        if conv ~= 1
            % %         %%%%% USING FSOLVE
            h = @(X)ComputeInflowDerivatives(X, v_loc_BA,w_loc_BA,N,BETA,PSI_IN,PSIW,Vhub_RA, Om_RA, rho, SpeedOfSound,Setup);

            %         options = optimoptions('fsolve','Display','off','TolX',0.01,'Algorithm','trust-region-dogleg', 'MaxFunctionEvaluations',200, 'MaxIterations',100,'FunctionTolerance',1e-3);

            options = optimoptions('fsolve','Display','off','UseParallel',false,'FunctionTolerance',1e-3');

            x_infl0 = zeros(7,nRotorLocal);
            x_infl0(1,:) = Setup.InflowGuess;

            [X,FVAL,EXITFLAG,OUTPUT] = fsolve(h,x_infl0,options);

            x_infl = X;
            dx_infl = FVAL;
            iter = norm(FVAL);% OUTPUT.iterations;
            conv = EXITFLAG/10;
        end


function [Xr_dot] = ComputeInflowDerivatives(x_infl, v_loc_BA,w_loc_BA,N,BETA,PSI_IN,PSIW,Vhub_RA, Om_RA, rho, SpeedOfSound,Setup)
                radius = Setup.RotorRadius;
                [~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,~,loads_PPDIM,~]  = ComputeRotorLoads(v_loc_BA,w_loc_BA,N,BETA,x_infl,PSI_IN,PSIW,rho,SpeedOfSound,Setup);
                [Xr_dot,Xr_ss] = EvalDynamicInflowModel(x_infl,Vhub_RA,Om_RA,loads_PPDIM,N,radius);
end
...
% FUNCTION TO COMPUTE ROTOR THRUST FOR A GIVEN INFLOW
    function [netThrust,rotorThrust,ductThrust,dT,dL,dD,Cl,Cd,ALPHADEG,phi,RE,v_in] = ComputeThrust(v_loc_BA,w_loc_BA,N,beta,x_inflow,sin_PSI_IN,cos_PSI_IN,rho,SpeedOfSound,Setup)
        ...
        v_in = vi0 + (Setup.R/Setup.RotorRadius).*(v1s.*sin_PSI_IN + v1c.*cos_PSI_IN);
...
%         MachEff
        ...
        etastar = Setup.etastar;
        lamstar = (w_loc_BA+v_in)./(abs(N)*Setup.RotorRadius);
        fstar = (Setup.nBlades/2).*(1-etastar)./lamstar;
        F0star = (2/pi)*acos(exp(-fstar));
        
        F0 = min(1,F0./F0star);        


        CalibFactor = Setup.Factor;              
                
        
        F = min(1,F0.*CalibFactor);
        ...
        CL0 = Setup.AirfoilDef.CL0;
        CL_a = Setup.AirfoilDef.CL_a;
        CL_min = Setup.AirfoilDef.CL_min;
        CL_max = Setup.AirfoilDef.CL_max;
        CD0 = Setup.AirfoilDef.CD0;
        CLCD0 = Setup.AirfoilDef.CLCD0;
        REexp = Setup.AirfoilDef.REexp;
        REref = Setup.AirfoilDef.REref;
        CD2u = Setup.AirfoilDef.CD2u;
        CD2l = Setup.AirfoilDef.CD2l;
...
% FUNCTION TO COMPUTE ROTOR LOADS FOR A GIVEN INFLOW
    function [NetThrust,RotorThrust,DuctThrust,dT,dL,dD,Cl,Cd,ALPHADEG,PHI,RE,F_RA,M_RA,Fx_RA,Fy_RA,Fz_RA,Mx_RA,My_RA,Mz_RA,Trq,dFx_BA,dFy_BA,dFz_BA,dMx_RA,dMy_RA,dMz_RA,Loads_PPDIM,v_in]  = ComputeRotorLoads(v_loc_BA,w_loc_BA,N,BETA,XInflow,PSI_IN,PSIW,rho,SpeedOfSound,Setup)
        ...
        % compute sin and cos 
        sin_PSI_IN = sin(PSI_IN);
        cos_PSI_IN = cos(PSI_IN);

        % Either with the given inflow or the computed inflow, compute thrust:
        [NetThrust,RotorThrust,DuctThrust,dT,dL,dD,Cl,Cd,ALPHADEG,PHI,RE,v_in] = ComputeThrust(v_loc_BA,w_loc_BA,N,BETA,XInflow,sin_PSI_IN,cos_PSI_IN,rho,SpeedOfSound,Setup);
        ...
        dFz_BA = -dT; %-(dL.*cos(PHI) - dD.*sin(PHI));
        ...
        Fz_RA = Setup.ThrustScaler * nBlades * sum(sum(dFz_RA))/npsi  + Fz_RA_Duct + Fzvane_RA;
        Mx_RA = nBlades * sum(sum(dMx_RA))/npsi  + Mx_RA_Duct + Mxvane_RA;
        My_RA = nBlades * sum(sum(dMy_RA))/npsi  + My_RA_Duct + Myvane_RA;
        Mz_RA = Setup.TorqueScaler * nBlades * sum(sum(dMz_RA))/npsi  + Mz_RA_Duct + Mzvane_RA;
