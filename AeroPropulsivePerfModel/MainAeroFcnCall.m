function [dFx_AC_b,dFy_AC_b,dFz_AC_b,dMx_AC_b,dMy_AC_b,dMz_AC_b,AOA_eff] = MainAeroFcnCall(a,rho,Vehicle,V_s,AOA_dir,Controls,th_ROM)
...
% Blade Pitch
CPBladePitch = zeros(4,1); %Controls(19:22);
LPBladePitch = zeros(2,1); %Controls(23:24);
...
% [deltaCl_LWing, detlaCir_LWing] = EMPAERO_CalcDeltaClCS(Vehicle.Aero.LWing, [dF3, dF2, dF1], V_s(LWIndices), c_strip(LWIndices), Vehicle);
% [deltaCl_RWing, detlaCir_RWing] = EMPAERO_CalcDeltaClCS(Vehicle.Aero.RWing, [dF4, dF5, dF6], V_s(RWIndices), c_strip(RWIndices), Vehicle);
...
deltaCl_LWing(OBDLWFlaperon) = 0;
deltaCl_RWing(OBDRWFlaperon) = 0;
