...
C
 1000 FORMAT(1X,78('-'))
 1050 FORMAT(/,1X,'Parent Airfoils', 25X,
...
C
 1000 FORMAT(1X,78('-'))
 1010 FORMAT( ' Lofted Blade: ',A32, 16X, A16)
 1035 FORMAT( 
...
C
 1106 FORMAT(
     &    ' StationSpec    :',I4,7X, 'Density-hub/tip:',F6.2,
     &  /,' TipRegion -blds:',F6.2,5X,'TipRefinement  :',F6.2,5X,
     &    'Dihedral -tip mm: ',F6.2,
     &  /,' ThickSpecHub-mm:',F6.2,5X,'OverShootHub-mm:',F6.2,5X,
     &    'PitchAxisHub-x/c: ',F6.3,
     &  /,' ThickSpecTip   :',F6.2,5X,'TipCut      -mm:',F6.2,5X,
     &    'PitchAxisTip    : ',F6.3)
C
...
C
 1120 FORMAT(
     ...
     &       '      Coefficient:',G13.4)
C
 1200 FORMAT(  ' Stn Rad_cm Rad_in Cd_cm Cd_in  Bxrot',
...
C          XXFFFFFXXFFFFFFXFFFFFFXFFFFFFXFFFFFFXXFFFFFF
 1220 FORMAT(1X,I2,A2,F6.2,1X,F6.2,1X,F5.2,1X,F5.2,2X,F5.2,
     &    2X,F5.2,1X,F6.2,F6.2,1X,F6.2,1X,F6.3,2X,F6.4)
C
 1225 FORMAT(1X,I2,A2,F6.2,1X,F6.2,1X,F5.2,1X,F5.2)
C
 1230 FORMAT(A50,15X,A20,/)
C
 1240 FORMAT(A50,/)
 1250 FORMAT(1X,A14, 40X, A23)
C
 1260 FORMAT(' Circular Root Blend      Length/bladelength : ',F6.3,
     &      3X,' Root radius: ',F6.2,' mm',
     &       /,' Chord parameter:',F5.1,
     &      3X,' Thickness parameter: ',F5.1,
     &      4X,' Beta relax : ',F5.2,' deg')
C
       RETURN
       END  ! PRINTLOFT
C
