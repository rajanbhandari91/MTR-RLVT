function [Fus] =  GeomEval_Fuselage1(Fus)
global Settings
...
for i = 1:ncs
    ...
    ns(i) = max(10,ceil(Fus.CrossSections(i).Length/Fus.MaxDistBetweenStations));
...
Fus.Cross = interpolateCS(Fus,nfs,CS_ind,fs);

for i = 1:nfs
    
    % CS = CSDef(CS_ind(i)).CS;
    CS = Fus.Cross(i).CS;
    % CS = Fus.CrossSections(CS_ind(i)).CS;
