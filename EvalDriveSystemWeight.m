...
% W_blade     = weight of single rotor blade {lb}
...
P_DSlimit = X(1); % drive system power limit (MCP) {hp}
N_rotor = X(2); % number of main rotors
N_gb = X(3); % number of gear boxes
N_ds = X(4); % number of intermediate drive shafts
Omega_rotor = X(5); % main-rotor rotation speed {rpm}
Omega_eng = X(6); % engine output speed {rpm}
x_hub = X(7); % length of drive shaft between motors
V_tip = X(8); % main rotor tip speed {ft/s}
W_blade = X(9);

f_Q = X(10); % second (main or tail) rotor torque limit (fraction of total drive system)
f_P = X(11); % second (main or tail) rotor power limit (fraction of total drive system)
f_rs = X(12); % rotor shaft weight (fraction gear box and rotorshaft) (= 0.13, range 0.06-0.20)

X_gb = X(13); % gear box factor (1 if present, else 0)
X_rs = X(14); % rotor shaft factor (1 if present, else 0)
X_ds = X(15); % drive shaft factor (1 if present, else 0)
X_rb = X(16); % rotor brake factor (1 if present, else 0)
